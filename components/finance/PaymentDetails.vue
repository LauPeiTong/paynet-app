  ...
  v-card.rounded-lg(outlined)
    v-list-item.pt-2(three-line)
      v-list-item-content.text-center
        v-list-item-subtitle.subtitle-1.darkGrey--text Tero Point Gained
            ...
            v-row.px-2.mb-4.mt-0.justify-center
              v-row
                ...
                v-col.pt-2.pb-0.d-flex.flex-column.px-0(:cols="5")
                  ...
                  v-chip.mr-auto.mb-2(v-if="item.point > 0"  color="primary" outlined pill small) {{ item.point }} Teco Points
...
export default {
  ...
  data () {
    return {
      selectedIndex: 1,
      payments: [
        { id: 1, name: 'Water Bottle', price: 2350, unitPrice: 1175, icon: 'star', color: '#237ABC', quantity: 2, point: 20 },
        { id: 2, name: 'Potato Chip', price: 480, unitPrice: 480, icon: 'cube', color: 'grey lighten-1', quantity: 1, point: 0 },
        { id: 3, name: 'LED light bulbs', price: 3840, unitPrice: 960, icon: 'star', color: '#237ABC', quantity: 4, point: 48 }
      ]
    }
  },
  computed: {
